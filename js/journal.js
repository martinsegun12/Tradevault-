...
function calculateTradeMetrics() {
    ...
    const pipMultiplier = isJPY ? 100 : isIndex ? 1 : isCrypto ? 100000 : 10000;
    ...
    // Calculate P&L (simplified: $10 per pip per standard lot)
    const pnl = pips * lot * 10;

    // Calculate R:R
    let rr = 'N/A';
    if (sl && tp) {
        const risk = Math.abs(entry - sl) * pipMultiplier;
        const reward = Math.abs(tp - entry) * pipMultiplier;
        if (risk > 0) {
            rr = '1:' + (reward / risk).toFixed(1);
        }
    }

    document.getElementById('trade-pips').value = pips.toFixed(1);
    document.getElementById('trade-pnl').value = Math.round(pnl);
...
async function saveTrade(e) {
    ...
    try {
        ...
        for (const img of uploadedImages) {
            const ref = storage.ref(`trades/${currentUser.uid}/${Date.now()}_${img.file.name}`);
        ...
        if (userDoc.exists) {
            ...
            userRef.update({
                ...
                lossCount: (data.lossCount || 0) + (!isWin ? 1 : 0),
